...
describe('integration tests', () => {
  describe.each(['1.0', '2.0'])('event version %s', (version) => {
    beforeAll(() => {
      app.loadTestData()
    })
    ...
    function makeEvent (eventOverrides) {
      ...
      if (eventOverrides.path) {
        root.rawPath = eventOverrides.path
      }
      if (eventOverrides.httpMethod) {
        root.requestContext.http.method = eventOverrides.httpMethod
      }
      return root
    }
